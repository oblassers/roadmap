...
class OrgTest < ActiveSupport::TestCase
  setup do
    @org = create(:org, name: 'Testing', abbreviation: 'TST')
  ...
  test "published_templates should return all published templates" do
    3.times do |i|
      template = build(:template, org: @org, version: 1, title: "Testing #{i}")
      if i < 2
  ...
  # ---------------------------------------------------
  test "can CRUD" do
    org = create(:org, name: 'testing', links: {"org":[]})
  ...
  test "can manage has_many relationship with TokenPermissionTypes" do
    tpt = build(:token_permission_type)
  ...
  test "Guidance Group should be created after_create of the Org" do
    org = create(:org)
    assert_equal org.reload.guidance_groups.count, 1
    org.destroy!
  end
end
